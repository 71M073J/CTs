﻿import { Cost, CustomCost, ExponentialCost, FreeCost, LinearCost, StepwiseCost } from "./api/Costs";
...
var currency;
var t = 0.0;
var f, c1, c2, q, q1, p, dt, tickCnt;
var dtMilestone, c1Exp, c2Exp;
var dts = [0.1, 0.05, 0.01]
...
var init = () => {
...

    {
        ...
        c2 = theory.createUpgrade(3, currency, new ExponentialCost(1, 2));
    ...
    // Permanent Upgrades
    {
        p1 = theory.createPublicationUpgrade(0, currency, 1e1);
        p1.boughtOrRefunded = (_) => {resetToPIMult();};
    }
    {
        p2 = theory.createBuyAllUpgrade(1, currency, 1e1);
        p1.boughtOrRefunded = (_) => resetToPIMult();
    }
    {
        p3 = theory.createAutoBuyerUpgrade(2, currency, 1e1);
        p1.boughtOrRefunded = (_) => resetToPIMult();
    }
    ///////////////////////
    //// Milestone Upgrades
    ...
    {
        dtMilestone = theory.createMilestoneUpgrade(0, 1);
        dtMilestone.description = Localization.getUpgradeMultCustomDesc("dt", "0.2");
        dtMilestone.info = Localization.getUpgradeMultCustomDesc("dt", "0.2");
...
var trimSciNota = (n) => {
    ...
    if (temp[0][0] == "0"){
        ...
        return temp[0].substring(0,4) + "e" + temp[1];
    }
}
var postPublish = () => {
    t = 0.0;
...
var getSecondaryEquation = () => {
    ...
    result += "q={" + q + "}";
    result += "\\ ";
    result += "\\dot q={q_1q_2}/1000 ={" + (getQ1(q1.level) * getQ2(q2.level) / 1e3) + "}";
    result += "\\end{matrix}";
    return result
}
...
//if you just need to seralise "t", here's an example: quoth philles gillipe
